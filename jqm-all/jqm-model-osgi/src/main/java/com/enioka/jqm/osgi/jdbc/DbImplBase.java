...
/**
 * All the SQL templates used in the application. Templates are as pure ANSI 92 SQL as possible, if not are HSQLDB inspired and pass through
 * a {@link DbAdapter} to run on specific databases.
 */
class DbImplBase
{
        /**
        ...
        {
                // VERSION
                queries.put("version_insert",
                                "INSERT INTO __T__VERSION(ID, COMPONENT, VERSION_D1, COMPAT_D1, INSTALL_DATE) VALUES(JQM_PK.nextval, 'SCHEMA', ?, ?, CURRENT_TIMESTAMP)");
                ...
                queries.put("node_update_all_enable_ws",
                                "UPDATE __T__NODE SET LOAD_API_SIMPLE=true, LOAD_API_CLIENT=true, LOAD_API_ADMIN=true, DNS='0.0.0.0'");
                queries.put("node_update_enable_ws_by_id",
                                "UPDATE __T__NODE SET LOAD_API_SIMPLE=true, LOAD_API_CLIENT=true, LOAD_API_ADMIN=true, DNS='0.0.0.0' WHERE ID=?");
                queries.put("node_update_all_disable_ws", "UPDATE __T__NODE SET LOAD_API_CLIENT=false, LOAD_API_ADMIN=false");
                queries.put("node_update_all_disable_all_ws",
                                "UPDATE __T__NODE SET LOAD_API_SIMPLE=false, LOAD_API_CLIENT=false, LOAD_API_ADMIN=false");
                ...
                queries.put("node_update_changed_by_id",
                                "UPDATE __T__NODE SET REPO_DELIVERABLE=?, DNS=?, ENABLED=?, JMX_REGISTRY_PORT=?, JMX_SERVER_PORT=?, "
                                                + "LOAD_API_ADMIN=?, LOAD_API_CLIENT=?, LOAD_API_SIMPLE=?, NAME=?, PORT=?, REPO_JOB_DEF=?, ROOT_LOG_LEVEL=?, STOP=?, REPO_TMP=? "
                ...
                queries.put("q_update_changed_by_id",
                                "UPDATE __T__QUEUE SET DEFAULT_QUEUE=?, DESCRIPTION=?, NAME=? WHERE ID=? AND NOT (DEFAULT_QUEUE=? AND DESCRIPTION=? AND NAME=?)");
                ...
                // DEPLOYMENT
                queries.put("dp_insert",
                                "INSERT INTO __T__QUEUE_NODE_MAPPING(ID, ENABLED, LAST_MODIFIED, MAX_THREAD, POLLING_INTERVAL, NODE, QUEUE) VALUES(JQM_PK.nextval, ?, CURRENT_TIMESTAMP, ?, ?, ?, ?)");
                ...
                queries.put("dp_update_changed_by_id",
                                "UPDATE __T__QUEUE_NODE_MAPPING SET ENABLED=?, LAST_MODIFIED=CURRENT_TIMESTAMP, MAX_THREAD=?, POLLING_INTERVAL=?, NODE=?, QUEUE=? WHERE ID=? AND NOT "
                                                + "(ENABLED=? AND MAX_THREAD=? AND POLLING_INTERVAL=? AND NODE=? AND QUEUE=?)");
                queries.put("dp_select_by_id",
                                "SELECT ID, ENABLED, LAST_MODIFIED, MAX_THREAD, POLLING_INTERVAL, NODE, QUEUE FROM __T__QUEUE_NODE_MAPPING WHERE ID=?");
                queries.put("dp_select_for_node",
                ...
                queries.put("dp_select_sum_queue_capacity",
                                "SELECT SUM(dp.MAX_THREAD) FROM __T__QUEUE_NODE_MAPPING dp LEFT JOIN __T__NODE n ON n.ID = dp.NODE WHERE dp.ENABLED = true AND n.ENABLED = true AND dp.QUEUE = ?");
                queries.put("dp_select_all_with_names",
                                "SELECT dp.ID, dp.ENABLED, dp.LAST_MODIFIED, dp.MAX_THREAD, dp.POLLING_INTERVAL, dp.NODE, dp.QUEUE, n.NAME, q.NAME FROM __T__QUEUE_NODE_MAPPING dp LEFT JOIN __T__NODE n ON n.ID=dp.NODE LEFT JOIN __T__QUEUE q ON q.ID=dp.QUEUE ");
                ...
                // CL
                queries.put("cl_insert",
                                "INSERT INTO __T__CL(ID, NAME, CHILD_FIRST, HIDDEN_CLASSES, TRACING, PERSISTENT, ALLOWED_RUNNERS) VALUES(JQM_PK.nextval, ?, ?, ?, ?, ?, ?)");
                queries.put("cl_delete_all", "DELETE FROM __T__CL");
                queries.put("cl_delete_by_id", "DELETE FROM __T__CL WHERE ID=?");
                queries.put("cl_update_all_fields_by_id",
                                "UPDATE __T__CL SET NAME=?, CHILD_FIRST=?, HIDDEN_CLASSES=?, TRACING=?, PERSISTENT=?, ALLOWED_RUNNERS=? WHERE ID=?");
                queries.put("cl_select_all",
                ...
                queries.put("clehprm_insert",
                                "INSERT INTO __T__CL_HANDLER_PARAMETER(ID, KEYNAME, VALUE, CL_HANDLER) VALUES(JQM_PK.nextval, ?, ?, ?)");
                ...
                queries.put("clehprm_delete_all_for_cl",
                                "DELETE FROM __T__CL_HANDLER_PARAMETER WHERE CL_HANDLER IN(SELECT h.ID FROM CL_HANDLER h WHERE h.CL=?)"); // subquery
                                                                                                                                          // -
                                                                                                                                          // better
                                                                                                                                          // multi
                                                                                                                                          // db
                ...
                // JOB DEF
                queries.put("jd_insert",
                                "INSERT INTO __T__JOB_DEFINITION(ID, APPLICATION, JD_KEY, CL, "
                ...
                queries.put("jd_update_all_fields_by_id", "UPDATE __T__JOB_DEFINITION SET APPLICATION=?, JD_KEY=?, "
                                + "DESCRIPTION=?, ENABLED=?, EXTERNAL=?, HIGHLANDER=?, "
                                + "PATH=?, CLASS_NAME=?, JAVA_OPTS=?, KEYWORD1=?, KEYWORD2=?, KEYWORD3=?, ALERT_AFTER_SECONDS=?, "
                                + "MODULE=?, PATH_TYPE=?, CL=?, QUEUE=? " + "WHERE ID=?");
                ...
                queries.put("jd_select_all",
                                "SELECT ID, APPLICATION, JD_KEY, CL, " + "DESCRIPTION, ENABLED, EXTERNAL, HIGHLANDER, "
                                                + "PATH, CLASS_NAME, JAVA_OPTS, KEYWORD1, KEYWORD2, KEYWORD3, ALERT_AFTER_SECONDS, "
                ...
                // JOB DEF PRM
                queries.put("jdprm_insert",
                                "INSERT INTO __T__JOB_DEFINITION_PARAMETER(ID, KEYNAME, VALUE, JOBDEF) VALUES(JQM_PK.nextval, ?, ?, ?)");
                ...
                // SCHEDULED JOBS
                queries.put("sj_insert",
                                "INSERT INTO __T__JOB_SCHEDULE(ID, CRON_EXPRESSION, JOBDEF, QUEUE, PRIORITY, LAST_UPDATED) VALUES(JQM_PK.nextval, ?, ?, ?, ?, CURRENT_TIMESTAMP)");
                ...
                queries.put("sjprm_insert",
                                "INSERT INTO __T__JOB_SCHEDULE_PARAMETER(ID, KEYNAME, VALUE, JOB_SCHEDULE) VALUES(JQM_PK.nextval, ?, ?, ?)");
                ...
                queries.put("sjprm_delete_all_for_jd",
                                "DELETE FROM __T__JOB_SCHEDULE_PARAMETER WHERE JOB_SCHEDULE IN (SELECT js.ID FROM __T__JOB_SCHEDULE js WHERE js.JOBDEF = ?)");
                queries.put("sjprm_select_all", "SELECT ID, KEYNAME, VALUE, JOB_SCHEDULE FROM __T__JOB_SCHEDULE_PARAMETER ");
                queries.put("sjprm_select_for_sj_list",
                                queries.get("sjprm_select_all") + " WHERE JOB_SCHEDULE IN(UNNEST(?)) ORDER BY JOB_SCHEDULE, ID");
                ...
                queries.put("ji_delete_waiting_in_queue_id",
                                "DELETE FROM __T__JOB_INSTANCE WHERE QUEUE = ? AND STATUS IN ('HOLDED', 'SUBMITTED', 'SCHEDULED')");
                queries.put("jj_update_cancel_by_id",
                                "UPDATE __T__JOB_INSTANCE SET STATUS='CANCELLED' WHERE ID=? AND (STATUS='SUBMITTED' OR STATUS='SCHEDULED' OR STATUS='HOLDED')");
                ...
                queries.put("jj_update_notbefore_by_id",
                ...
                queries.put("jj_update_run_by_id",
                                "UPDATE __T__JOB_INSTANCE SET DATE_START=CURRENT_TIMESTAMP, STATUS='RUNNING' WHERE ID=? AND (STATUS='ATTRIBUTED' OR STATUS='RUNNING')");
                ...
                queries.put("ji_select_current_pos",
                ...
                queries.put("ji_select_existing_highlander_2",
                                "SELECT COUNT(1) FROM __T__JOB_INSTANCE WHERE JOBDEF=? AND STATUS IN('ATTRIBUTED', 'RUNNING')");
                queries.put("ji_select_changequeuepos_by_id",
                ...
                queries.put("ji_select_cnx_data_by_id",
                                "SELECT DNS||':'||PORT AS HOST FROM __T__JOB_INSTANCE ji LEFT JOIN __T__NODE n ON ji.NODE = n.ID WHERE ji.ID=?");
                queries.put("ji_select_instructions_by_node",
                                "SELECT ji.ID, ji.INSTRUCTION FROM __T__JOB_INSTANCE ji WHERE ji.STATUS='RUNNING' AND ji.INSTRUCTION <> 'RUN' AND ji.NODE=?");

                queries.put("ji_update_delayed",
                                "UPDATE __T__JOB_INSTANCE SET STATUS='SUBMITTED' WHERE STATUS='SCHEDULED' AND DATE_NOT_BEFORE <= CURRENT_TIMESTAMP");
                queries.put("ji_select_poll", queries.get("ji_select_all")
                                + " WHERE ji.QUEUE = ? AND ji.STATUS='SUBMITTED' ORDER BY ji.PRIORITY DESC, ji.INTERNAL_POSITION");
                queries.put("ji_update_status_by_id",
                                "UPDATE __T__JOB_INSTANCE SET STATUS='ATTRIBUTED', NODE=? WHERE STATUS='SUBMITTED' AND ID=?");
                ...
                queries.put("history_insert_with_end_date", "INSERT INTO __T__HISTORY(ID, JD_APPLICATION, JD_KEY, DATE_ATTRIBUTION, EMAIL, "
                                ...
                                + "VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                queries.put("history_insert", queries.get("history_insert_with_end_date").replace("(?, ?, ?, ?, ?, ?",
                                "(?, ?, ?, ?, ?, CURRENT_TIMESTAMP"));
                ...
                queries.put("history_select_count_last_mn_for_poller",
                                "SELECT COUNT(1)/60 FROM __T__HISTORY WHERE QUEUE=? AND NODE=? AND DATE_END > (CURRENT_TIMESTAMP - 1 MINUTE)");
                ...
                // DELIVERABLE
                queries.put("deliverable_insert",
                                "INSERT INTO __T__DELIVERABLE(ID, FILE_FAMILY, PATH, JOB_INSTANCE, ORIGINAL_FILE_NAME, RANDOM_ID) VALUES(JQM_PK.nextval, ?, ?, ?, ?, ?)");
                queries.put("deliverable_delete_all", "DELETE FROM __T__DELIVERABLE");
                queries.put("deliverable_select_all",
                ...
                // RUNTIME PRM
                queries.put("jiprm_insert",
                                "INSERT INTO __T__JOB_INSTANCE_PARAMETER(ID, JOB_INSTANCE, KEYNAME, VALUE) VALUES(JQM_PK.nextval, ?, ?, ?)");
                ...
                // JNDI
                queries.put("jndi_insert",
                                "INSERT INTO __T__JNDI_OBJECT_RESOURCE(ID, AUTH, DESCRIPTION, FACTORY, LAST_MODIFIED, NAME, SINGLETON, TEMPLATE, TYPE) VALUES(JQM_PK.nextval, 'CONTAINER', ?, ?, CURRENT_TIMESTAMP, ?, ?, ?, ?)");
                ...
                queries.put("jndi_update_changed_by_id",
                                "UPDATE __T__JNDI_OBJECT_RESOURCE SET AUTH=?, DESCRIPTION=?, FACTORY=?, LAST_MODIFIED=CURRENT_TIMESTAMP, NAME=?, SINGLETON=?, TEMPLATE=?, TYPE=? WHERE ID=? AND NOT (AUTH=? AND DESCRIPTION=? AND FACTORY=? AND NAME=? AND SINGLETON=? AND TEMPLATE=? AND TYPE=?)");
                queries.put("jndi_select_count_for_key", "SELECT COUNT(1) FROM __T__JNDI_OBJECT_RESOURCE WHERE NAME=?");
                queries.put("jndi_select_count_changed",
                ...
                // JNDI PRM
                queries.put("jndiprm_insert",
                                "INSERT INTO __T__JNDI_OR_PARAMETER(ID, KEYNAME, LAST_MODIFIED, VALUE, JNDI_OR) VALUES(JQM_PK.nextval, ?, CURRENT_TIMESTAMP, ?, ?)");
                ...
                queries.put("jndiprm_update_value_by_key",
                                "UPDATE __T__JNDI_OR_PARAMETER SET VALUE=?, LAST_MODIFIED=CURRENT_TIMESTAMP WHERE KEYNAME=?");
                queries.put("jndiprm_update_changed_by_id",
                                "UPDATE __T__JNDI_OR_PARAMETER SET VALUE=?, LAST_MODIFIED=CURRENT_TIMESTAMP WHERE JNDI_OR=? AND KEYNAME=? AND NOT (VALUE=?)");
                queries.put("jndiprm_select_all_in_jndisrc",
                                "SELECT ID, KEYNAME, LAST_MODIFIED, VALUE FROM __T__JNDI_OR_PARAMETER WHERE JNDI_OR=?");
                queries.put("jndiprm_select_all_in_jndisrc_list",
                ...
                queries.put("role_select_all_for_user",
                                "SELECT r.ID, r.NAME, r.DESCRIPTION FROM __T__RROLE r RIGHT JOIN __T__RROLE_RUSER a ON a.ROLE = r.ID WHERE a.ACCOUNT=?");
                queries.put("role_select_id_for_user_list",
                                "SELECT a.ROLE, a.ACCOUNT FROM __T__RROLE_RUSER a WHERE a.ACCOUNT IN(UNNEST(?))");
                ...
                // R-USER
                queries.put("user_insert",
                                "INSERT INTO __T__RUSER(ID, CREATION_DATE, EMAIL, EXPIRATION_DATE, FREETEXT, HASHSALT, INTERNAL, LAST_MODIFIED, LOCKED, LOGIN, PASSWORD) VALUES(JQM_PK.nextval, CURRENT_TIMESTAMP, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, ?, ?)");
                queries.put("user_delete_all", "DELETE FROM __T__RUSER");
                queries.put("user_delete_by_id", "DELETE FROM __T__RUSER WHERE ID=?");
                queries.put("user_delete_expired_internal",
                                "DELETE FROM __T__RUSER WHERE internal=true AND EXPIRATION_DATE < CURRENT_TIMESTAMP");
                queries.put("user_add_role_by_name",
                                "INSERT INTO __T__RROLE_RUSER(ID, ACCOUNT, ROLE) VALUES(JQM_PK.nextval, ?, (SELECT r.ID FROM __T__RROLE r WHERE r.NAME=?))");
                ...
                queries.put("user_update_changed",
                                "UPDATE __T__RUSER SET LOGIN=?, LOCKED=?, EXPIRATION_DATE=?, LAST_MODIFIED=CURRENT_TIMESTAMP, EMAIL=?, FREETEXT=? WHERE ID=? AND NOT (LOGIN=? AND LOCKED=? AND EXPIRATION_DATE=? AND EMAIL=? AND FREETEXT=? )");
                queries.put("user_select_all_in_role",
                ...
                // GLOBAL PRM
                queries.put("globalprm_insert",
                                "INSERT INTO __T__GLOBAL_PARAMETER(ID, KEYNAME, VALUE, LAST_MODIFIED) VALUES(JQM_PK.nextval, ?, ?, CURRENT_TIMESTAMP)");
                queries.put("globalprm_update_value_by_key",
                                "UPDATE __T__GLOBAL_PARAMETER SET VALUE=?, LAST_MODIFIED=CURRENT_TIMESTAMP WHERE KEYNAME=?");
                ...
                queries.put("globalprm_select_count_modified_jetty",
                ...
                // WITNESS
                queries.put("w_insert",
                                "INSERT INTO __T__WITNESS(ID, KEYNAME, NODE, LATEST_CONTACT) VALUES(JQM_PK.nextval, 'SCHEDULER', ?, CURRENT_TIMESTAMP)");
                queries.put("w_update_take",
                                "UPDATE __T__WITNESS SET NODE=?, LATEST_CONTACT=CURRENT_TIMESTAMP WHERE KEYNAME='SCHEDULER' AND (LATEST_CONTACT IS NULL OR NODE IS NULL OR NODE=? OR (NODE<>? AND LATEST_CONTACT < (CURRENT_TIMESTAMP - ? SECOND)))");
        }
